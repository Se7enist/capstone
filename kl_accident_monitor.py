...
# Data file path - Kuala Lumpur historical data
KL_ACCIDENT_DATA_FILE = "C:\\Users\\Luo\\Desktop\\kl_accidents.csv"

# Kuala Lumpur boundaries
KL_BOUNDS = {
    'min_lat': 2.9500,    # 吉隆坡南部边界
    'max_lat': 3.3500,    # 吉隆坡北部边界
    'min_lon': 101.5000,  # 吉隆坡西部边界
    'max_lon': 101.8500   # 吉隆坡东部边界
}
...
def load_accident_data():
    ...
    if os.path.exists(KL_ACCIDENT_DATA_FILE):
        try:
            # Read historical data
            df = pd.read_csv(KL_ACCIDENT_DATA_FILE)
        ...
        st.error(f"Data file not found: {KL_ACCIDENT_DATA_FILE}")
        st.info("Please run filter_kl_accidents.py first to generate Kuala Lumpur data file")
        return pd.DataFrame()
...
def save_accident_data(accident_data):
    ...
    try:
        ...
        # Save back to file
        df.to_csv(KL_ACCIDENT_DATA_FILE, index=False)
...
def get_tomtom_incidents():
    ...
    try:
        ...
        bbox = f"{KL_BOUNDS['min_lon']},{KL_BOUNDS['min_lat']},{KL_BOUNDS['max_lon']},{KL_BOUNDS['max_lat']}"
...
def geocode_address(address):
    ...
    try:
        ...
        params = {
            ...
            'bounds': f"{KL_BOUNDS['min_lat']},{KL_BOUNDS['min_lon']}|{KL_BOUNDS['max_lat']},{KL_BOUNDS['max_lon']}"
        }
...
def get_place_suggestions(query):
    ...
    try:
        ...
        params = {
            'input': query,
            'key': GOOGLE_PLACES_KEY,
            'location': f"{3.1390},{101.6869}",  # Kuala Lumpur center
        ...
        if data['status'] == 'OK':
            ...
            for prediction in data['predictions']:
                # Only include suggestions that mention Kuala Lumpur or KL or Malaysia
                if any(term in prediction['description'].lower() for term in ['kuala lumpur', 'kl', 'malaysia', 'selangor']):
...
def page_realtime_monitoring():
    ...
    with tab2:
        ...
        if 'manual_lat' not in st.session_state:
            st.session_state.manual_lat = 3.1390
        if 'manual_lon' not in st.session_state:
            st.session_state.manual_lon = 101.6869
        ...
        st.markdown("**Enter address or place name in Kuala Lumpur:**")
        ...
        address_query = st.text_input(
            "Start typing an address (e.g., 'Petronas Tower', 'Bukit Bintang')", 
...
def main():
    ...
    with st.sidebar:
        ...
        if os.path.exists(KL_ACCIDENT_DATA_FILE):
            file_size = os.path.getsize(KL_ACCIDENT_DATA_FILE) / 1024 / 1024
